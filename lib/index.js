...
const createTempFile =
    async (content) => { return await tempFile(content, "temp.m"); };

const hasMATLAB = () => { return !shell.which("matlab"); };
...
const run = async (input) => {
  ...
  try {
    ...
    if (version) {
      fs.existsSync(input) ? (temporaryFile = input)
                           : (temporaryFile = await createTempFile(input));

      return new Promise((resolve, reject) => {
        exec(`matlab -nosplash -batch "run('${temporaryFile}'); exit;"`,
             (error, stdout, stderr) => {
               if (error) {
                 reject(stderr.trim());
               }
               resolve(stdout.replace("ans =\r\n\r\n", "")
                           .trim()
                           .replace(/\\n/g, "\n")
                           .trim());
             });
